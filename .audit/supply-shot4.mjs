// Force viewport via CDP, then full-page screenshot.
...
const edge = spawn(EDGE, [
  ...
  '--user-data-dir=' + resolve(__dirnameLocal, 'profile-supply4'),
...
console.log('content:', metrics.cssContentSize);
const shot = await cmd('Page.captureScreenshot', {
  format: 'png',
  captureBeyondViewport: true,
  clip: { x: 0, y: 0, width: Math.ceil(metrics.cssContentSize.width), height: Math.min(3000, Math.ceil(metrics.cssContentSize.height)), scale: 1 }
});
