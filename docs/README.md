...
> Companion deliverable: [`REPORT.md`](./REPORT.md) — long-form executive analysis, technical appendix and one-page Action Dashboard generated from the same dataset.
...
|---|---|---|
| **`start-dev.bat`** | Spins up MongoDB, the backend in **tsx watch** mode (live reload), and the Vite dev server (HMR). Opens 3 cmd windows + your browser at `http://localhost:5173/`. | Day-to-day editing / debugging |
| **`start-prod.bat`** | Builds the frontend (`vite build`) and backend (`tsc`), then starts MongoDB and a single Node server on `:4000` that serves **both** the API and the built frontend. Opens 2 cmd windows + browser at `http://localhost:4000/`. Pass `--rebuild` (or `-r`) to force a fresh build even if `dist/` exists. | Demo / "production" / share with non-devs |
| **`stop.bat`** | Closes every WTY window by title and frees ports 4000, 5173 and 27017. | When you're done |
...
├─ stop.bat                       # closes WTY windows + frees :4000 / :5173 / :27017
├─ .gitignore
